...
| 分类 | 道具 |
| --- | --- |
| 伤害 | 蟋蟀的头、殉道者之血、史蒂文、波吕斐摩斯、吐根 |
| 射速 | 悲伤洋葱、第一号、豆奶、内眼 |
| 子弹形态 | 科技（激光）、弯勺（追踪）、丘比特之箭（穿透）、吐根（爆炸） |
| 生存 | 早餐、魔法蘑菇、圣斗篷（每房免伤一次）、薄饼（减伤）、深渊领主（飞行） |
| 速度/辅助 | 急速球、皮带、鲍比兄弟（自动射击跟班） |
...
- 敌人 AI 差异化明显：冲脸 / 放风筝 / 站桩炮台 / 自杀袭击 / 免伤正面
...
**无头冒烟测试（`node test/smoke.js`）：7/7 通过**
- 地牢生成合法性：5 个 seed 验证房间数、恰 1 个 Boss 房、宝箱房数量
- BOT 全流程通关：3 层 + 最终 Boss，约 35 秒完成
...
**调试接口**：浏览器控制台可访问 `window.__game` 查看游戏状态（`state` / `player` / `enemies` / `room` / `floor` / `stats` / `dungeon`），或调用 `start(seed)`、`giveItem('polyphemus')`、`godMode()`、`killAll()`、`botStart()` 等快速验证。
...
js/dungeon.js       BFS 地牢生成 + 房间模板
js/items.js         20 件道具 + 道具池
js/entities.js      玩家/眼泪/6 种敌人 AI/3 个 Boss 状态机
