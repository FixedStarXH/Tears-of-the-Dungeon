...
const ITEMS = {
  ...
  // ---- p10 参考原作新增 ----
  twenty: { name: '20/20', icon: 'twenty', color: '#c8d8e8', stat: '双重射击', desc: '一次射出两颗眼泪（伤害略降）', effect(p) { p.doubleShot = true; } },
  mutant_spider: { name: '突变蜘蛛', icon: 'spider', color: '#7a5a4a', stat: '四重射击', desc: '一次射出四颗眼泪（伤害略降）', effect(p) { p.quadShot = true; } },
  loki: { name: '洛基之角', icon: 'loki', color: '#d8a048', stat: '四向齐射', desc: '25% 概率向四个方向齐射', effect(p) { p.loki = true; } },
  moms_contact: { name: '妈妈的爱抚', icon: 'moms', color: '#c8b8a8', stat: '减速', desc: '眼泪命中使敌人减速', effect(p) { p.momsContact = true; } },
  magneto: { name: '磁铁', icon: 'magnet', color: '#a8a8c0', stat: '磁吸', desc: '把地上的金币钥匙吸向自己', effect(p) { p.magneto = true; } },
  ouija: { name: '通灵板', icon: 'ouija', color: '#b8a8e0', stat: '幽灵泪', desc: '眼泪可以穿过岩石', effect(p) { p.ouija = true; } },
  proptosis: { name: '下垂症', icon: 'proptosis', color: '#e0a888', stat: '近强远弱', desc: '眼泪离敌人越近伤害越高', effect(p) { p.proptosis = true; } },
};
...
function applyItem(p, id, silent) {
  ...
  it.effect(p);
  // 全局属性钳制（参考原作 addItem 的 clamp）
  p.damage = clamp(p.damage, 0.3, 20);
  p.fireDelay = clamp(p.fireDelay, 2.5, 60);
  p.tearRange = clamp(p.tearRange, 120, 900);
  p.speed = clamp(p.speed, 60, 320);
  p.tearSpeed = clamp(p.tearSpeed, 180, 900);
  return it;
}
